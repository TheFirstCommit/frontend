...
function inferName(file: Props['file'], fallback = 'document.pdf') {
    if (typeof file === 'string') {
        try {
            const u = new URL(file, window.location.href);
...
export const PdfSlideViewer: React.FC<Props> = ({ file, onClose, title }) => {
    const [numPages, setNumPages] = useState(0);
    const onLoadSuccess = ({ numPages }: { numPages: number }) => setNumPages(numPages);

    const displayName = useMemo(() => title ?? inferName(file), [title, file]);
